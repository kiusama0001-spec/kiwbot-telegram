...
def _quick_keyboard(m):
    ...
    if typ=='draw':
        # Las misiones de dibujo usan la Main Mini App ya configurada del bot.
        # startapp conserva la identidad Telegram y qm_<id> solo enruta al lienzo;
        # no comparte estado con otros minijuegos ni modifica la creación normal.
        botname=str(get_bot_identity().get('username') or '').strip()
        if botname:
            return {"inline_keyboard":[[{"text":"🎨 Abrir lienzo","url":f"https://t.me/{botname}?startapp=qm_{mid}"}]]}
