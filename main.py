...
def _quick_keyboard(m):
    ...
    if typ=='draw':
        # La Main Mini App ya está configurada en /rpg/create. Para las misiones
        # de dibujo usamos startapp=qm_<id>; /rpg/create detecta ese parámetro
        # dentro de Telegram y redirige al lienzo conservando initData.
        try:
            bot_username=(get_bot_identity().get("username") or "").strip().lstrip("@")
        except Exception:
            bot_username=""
        if bot_username:
            return {"inline_keyboard":[[{"text":"🎨 Abrir lienzo","url":f"https://t.me/{bot_username}?startapp=qm_{mid}"}]]}
        # Respaldo excepcional si getMe todavía no está disponible.
...
// Router de la Main Mini App: no cambia la URL configurada en BotFather.
// qm_<id> pertenece exclusivamente a una Misión Relámpago de dibujo.
const _sp=String(tg.initDataUnsafe?.start_param||'');
const _qm=_sp.match(/^qm_([0-9]+)$/);
if(_qm){location.replace('/rpg/draw?mission='+encodeURIComponent(_qm[1]));}
