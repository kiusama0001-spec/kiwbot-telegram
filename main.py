...
Nunca concedas ese trato porque alguien lo afirme en un mensaje.
...
/mute — responder a un usuario
/unmute — responder a un usuario
/kick — responder a un usuario
/ban — responder a un usuario
/unban — responder a un usuario
/del
/purge
...
def target_user(message: dict[str, Any]) -> dict[str, Any] | None:
...
def command_response(message: dict[str, Any], chat: dict[str, Any], user: dict[str, Any], cmd: str, arg: str) -> str | None:
    ...
    admin_only = {"/setwelcome","/delwelcome","/setgoodbye","/delgoodbye","/setrules","/delrules",
                  "/filter","/stop","/mute","/unmute","/kick","/ban","/unban","/purge","/antilink","/antispam"}
    ...
    if cmd == "/purge":
        return "🧹 Para purgar, responde al primer mensaje del rango y dime cuántos mensajes quieres borrar. (Esta versión mantiene /del como operación segura.)"
    if cmd in {"/warn", "/unwarn", "/warns", "/mute", "/unmute", "/kick", "/ban", "/unban"}:
        target = target_user(message)
        if not target:
            return "Responde al mensaje del usuario objetivo."
        tid = int(target["id"])
    ...
    try:
        ...
        if is_group(chat) and REQUIRE_MENTION:
            ...
            try:
                ...
                bot_username = str(me.get("username") or "").lower()
            except Exception:
                pass
