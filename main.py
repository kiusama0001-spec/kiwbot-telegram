...
def send_rpg_image(chat_id, asset_key, caption="", reply_markup=None):
    ...
    if not path or not TELEGRAM_API:
        # Para monstruos del catálogo, crea el arte una sola vez y registra el file_id.
        # Cualquier fallo de Cloudflare se absorbe aquí para que el combate continúe en texto.
        if TELEGRAM_API and str(asset_key or "").startswith("enemy:"):
            try:
                return generate_and_register_enemy_art(
                    chat_id, asset_key, caption=caption, reply_markup=reply_markup,
                    message_thread_id=get_current_message_thread_id()
                )
            except Exception as e:
                logger.exception("No pude autogenerar arte RPG %s: %s", asset_key, e)
        return None
...
def cloudflare_generate_rpg_enemy_image(enemy_key):
    """Genera arte únicamente para monstruos REALES del catálogo RPG."""
    ...
    prompt=(
        "Official KiwRPG monster concept art, dark fantasy cinematic RPG, vertical portrait composition, "
        f"{name}, a hostile fantasy creature, full or near-full body clearly visible, centered main subject, "
        "ancient dangerous environment related to the creature, dramatic volumetric lighting, atmospheric depth, "
        "high detail, realistic fantasy materials, coherent dark blue and moonlit color language, premium game art, "
        "no text, no letters, no logo, no watermark, no interface, no frame"
    )
...
def generate_and_register_enemy_art(chat_id, asset_key, caption="", reply_markup=None, message_thread_id=None):
    """Genera una vez, sube a Telegram y persiste el file_id. Nunca regenera si ya existe."""
    asset_key=str(asset_key or "").strip().lower()
    if not asset_key.startswith("enemy:"):
        return None
    enemy_key=asset_key.split(":",1)[1].split(":",1)[0]
    if not _rpg_enemy_info(enemy_key):
        return None
    lock=_rpg_ai_lock(f"enemy:{enemy_key}")
    with lock:
        cache_key=f"img:enemy:{enemy_key}"
        cached=_rpg_asset_get(cache_key)
        if cached:
        ...
        if message_thread_id is None: message_thread_id=get_current_message_thread_id()
        if message_thread_id is not None: payload["message_thread_id"]=str(int(message_thread_id))
        ext="jpg" if "jpeg" in ctype else "png"
        resp=TELEGRAM_SESSION.post(
            f"{TELEGRAM_API}/sendPhoto",data=payload,
            files={"photo":(f"{enemy_key}.{ext}",raw,ctype)},timeout=TELEGRAM_TIMEOUT
        )
        data=resp.json() if resp.ok else {}
        if not resp.ok or not data.get("ok"):
            raise RuntimeError(f"Telegram rechazó el arte generado: {str(data or resp.text)[:500]}")
        ...
        logger.info("Arte IA registrado | asset=%s | bytes=%s",f"enemy:{enemy_key}",len(raw))
        return data
