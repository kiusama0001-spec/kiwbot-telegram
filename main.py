"""KiwBot: Telegram webhook bot backed by Groq via OpenAI-compatible API.
    ...
    def add(self, chat_id: int | str, role: str, text: str) -> None:
        ...
        with self._lock:
            self._messages[chat_key].append(
                MemoryMessage(
                    role=role,
                    text=cleaned[:MAX_STORED_MESSAGE_CHARS],
                )
            )
...
# ---------------------------------------------------------------------------
# Groq Client Initialization (OpenAI-compatible API)
# ---------------------------------------------------------------------------
...
if GROQ_API_KEY:
    try:
        ...
        logger.exception(
            "No se pudo inicializar Groq; se usará respuesta alternativa"
        )
else:
    logger.warning(
        "GROQ_API_KEY no está configurada; Groq queda desactivado"
    )
...
def generate_reply(
    ...
    try:
        ...
        messages = _history_as_openai_messages(
            chat_id,
            system_instruction,
        )
        ...
        logger.exception(
            "Error generando respuesta para chat %s",
            chat_id,
        )

        fallback = _address_owner(
            random.choice(FALLBACK_RESPONSES),
            user,
        )
...
def _fresh_command_text(
    ...
    with command_history_lock:
        ...
        index = random.choice(
            available or list(range(len(choices)))
        )
...
def _dice_fallback(
    ...
    if value == 1:
        ...
        mood = (
            "¡Un veinte natural! Hasta el azar decidió hacer su trabajo "
            "por una vez."
        )
    elif value >= 18:
        mood = (
            "Resultado excelente. Qué sospechoso; casi parece que sabías "
            "lo que hacías."
        )
    elif value <= 3:
        mood = (
            "Resultado desastroso. El dungeon ya está redactando la denuncia."
        )
    elif value <= 5:
        mood = (
            "Resultado flojo. La épica se tomó el día libre."
        )
    else:
        mood = (
            "Resultado decente. No legendario, pero tampoco una tragedia "
            "con piernas."
        )
...
def generate_dice_reply(
    ...
    try:
        ...
        if not answer:
            raise RuntimeError(
                "Groq devolvió una narración vacía"
            )
        ...
        logger.exception(
            "Error narrando dado para chat %s",
            chat_id,
        )

        return _address_owner(
            _dice_fallback(emoji, value),
            user,
        )
...
def split_message(
    text: str,
    max_rendered: int = TELEGRAM_MAX_RENDERED_CHARS,
) -> list[str]:
    ...
    while remaining:
        ...
        prefix_length = _largest_safe_prefix(
            remaining,
            max_rendered,
        )
...
recent_outbound_ids: dict[str, deque[int]] = defaultdict(
...
def _remember_outbound_message(
    ...
    with outbound_ids_lock:
        recent_outbound_ids[str(chat_id)].append(
            message_id
        )


def _was_our_message(
    chat_id: int | str,
    message_id: int | None,
) -> bool:
...
def send_long_message(
    ...
    for index, chunk in enumerate(chunks):
        ...
        try:
            ...
            logger.warning(
                "Markdown rechazado; reintentando texto plano: %s",
                markdown_error,
            )
            ...
            try:
                ...
                logger.warning(
                    "Reply rechazado; enviando sin reply: %s",
                    reply_error,
                )
...
def _reply_context(
    ...
    if isinstance(replied.get("text"), str):
        return (
            f"{_author_label(replied.get('from'))}: "
            f"{replied['text']}"
        )
    ...
    if isinstance(dice, dict):
        return (
            f"{_author_label(replied.get('from'))} lanzó "
            f"{dice.get('emoji')} y sacó {dice.get('value')}"
        )
    ...
    for field, kind, default_mime in (
        ...
        if isinstance(media, dict) and isinstance(
            ...
            return {
                ...
                "kind": kind,
                "mime_type": str(
                    media.get("mime_type") or default_mime
                ),
...
def _command_name(
    ...
    first = (
        text.strip().split(maxsplit=1)[0].lower()
        if text.strip()
        else ""
    )
...
def process_update(
    ...
    try:
        ...
        if media is not None:
            if (
                isinstance(media.get("file_size"), int)
                and media["file_size"] > MAX_MEDIA_BYTES
            ):
                answer = _address_owner(
                    MEDIA_TOO_LARGE_RESPONSE,
                    sender,
            ...
            try:
                ...
                logger.exception(
                    "Error manejando media para chat %s",
                    chat_id,
                )
...
def set_webhook(
    ...
    if parsed.scheme != "https" or not parsed.netloc:
        raise ValueError(
            "El webhook debe ser una URL HTTPS pública de Render"
        )
    ...
    try:
        ...
        if not result.get("ok"):
            raise RuntimeError(
                result.get(
                    "description",
                    "Telegram rechazó el webhook",
                )
            )

        logger.info(
            "Webhook de Telegram configurado automáticamente: %s",
            WEBHOOK_URL,
        )

    except Exception:
        logger.exception(
            "No se pudo configurar automáticamente el webhook de Telegram"
        )
...
def _cli() -> int:
    ...
    try:
        if command == "set-webhook":
            if len(sys.argv) != 3:
                print(
                    "Uso: python main.py set-webhook "
        ...
        if command == "webhook-info":
            ...
            print(
                {
                    "url": info.get("url", ""),
                    "pending_update_count": info.get(
                        "pending_update_count",
                        0,
                    ),
                    "last_error_date": info.get(
                        "last_error_date"
                    ),
                    "last_error_message": info.get(
                        "last_error_message"
                    ),
                }
            )
        ...
        print(
            "Comando no reconocido. Usa set-webhook o webhook-info."
        )
