...
def _omega_finalize(event,defeated=False):
    ...
    now=int(time.time())
    # Si Kenny fue derrotado, el evento termina inmediatamente.
    # El límite de 12 horas solo aplica cuando sigue con vida.
...
def process_command(
    ...
    if command in ("/encuentro", "/combatir"):
        ...
        if ok:
            ...
            kb=rpg_battle_keyboard(char["class_name"],0)
